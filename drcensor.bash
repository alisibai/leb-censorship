...
if [[ -n $cloudflare_DNS ]]; then
    ...
    echo "curl -v -m $timeout --resolve $domain:80:$cloudflare_DNS http://$domain"
    curl -v -m $timeout --resolve "$domain:80:$cloudflare_DNS" "http://$domain"
    [[ $? -ne 0 ]] && print_error "HTTP request failed"

    # Check HTTPS blocking
    print_step "Checking HTTPS (with Cloudflare's DoH)"
    echo "curl -v -m $timeout --resolve $domain:443:$cloudflare_DNS https://$domain"
    curl -v -m $timeout --resolve "$domain:443:$cloudflare_DNS" "https://$domain"
